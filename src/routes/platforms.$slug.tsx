...
export const Route = createFileRoute("/platforms/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const description =
      brand?.description ?? "Ray Lab Group diagnostic platform.";
    return {
      meta: [
        { title },
  ...
  return (
    <>
      ...
      <SectionShell bg="bg-background">
        ...
        {grouped.length === 0 ? (
          <div className="mt-10 rounded-2xl border border-dashed border-border/70 bg-card p-10 text-center text-sm text-muted-foreground">
